...
extra_all_lm_filtered <- lmer(log(price) ~ accommodates + review_scores_rating + room_type + host_is_superhost + period + relative_lat + 
                        relative_long + place + period*place + (1 | id), data = city_data)
extra_firenze_lm_filtered <- lmer(log(price) ~ accommodates + review_scores_rating + room_type + host_is_superhost + period + relative_lat + 
                        relative_long + (1 | id), data = firenze_data)
...
plot(fitted(extra_firenze_lm_filtered), resid(extra_firenze_lm_filtered), alpha = 0.5)
...
firenze_quantile <- lqmm(relative_price ~ 1,
                         random = ~ id, tau = 0.5, data = firenze_data)
...
robust_firenze <- rlmer(log(price) ~ accommodates + review_scores_rating + room_type + host_is_superhost + period + relative_lat + 
                        relative_long + (1 | id), data = firenze_data)
